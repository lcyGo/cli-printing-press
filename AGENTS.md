...
- Include the PR template's `## CLI Shape` section when the PR touches `catalog/*.yaml` or `catalog/specs/**`; show the generated command surface (`cli-printing-press generate <name>` top-level `--help` plus a command-to-endpoint/host table flagging novel/read-only commands). `validate-catalog.yml` requires real content for generatable entries (spec_url/spec_format or a file under `catalog/specs/`) and allows `N/A` for wrapper-only entries.
